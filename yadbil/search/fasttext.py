...
# TODO: change naming?
class FastTextWrapper(BaseSearch):
    def __init__(
        self,
        ...
        fasttext_params: Dict[str, Any] = None,
    ):
        ...
        if pretrained_emb_model_path is not None:
            self.embeddings = FastTextKeyedVectors.load(pretrained_emb_model_path)
            self.is_pretrained = True
        else:
            self.embeddings = FastText(**fasttext_params)
    ...
    def _embed_normalize_mean_normalize(self, words: List[str]) -> np.ndarray:
        emb = [self.embeddings[word] for word in words]
        emb = np.mean([x / np.linalg.norm(x) for x in emb], axis=0)
    ...
    def train(self, path_to_corpus: Union[str, Path] = None, data=None):
        ...
        corpus = JsonCorpus(path_to_corpus) if path_to_corpus is not None else JsonCorpus(data=data)
